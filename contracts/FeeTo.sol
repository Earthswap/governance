...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IEarthswapV2Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IEarthswapV2Pair(pair).token1()];
        ...
        if (
            ...
            value = IEarthswapV2Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IEarthswapV2Pair(pair).transfer(pair, value));
                IEarthswapV2Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IEarthswapV2Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}
